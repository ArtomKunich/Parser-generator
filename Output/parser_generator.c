...
void print_action_state(FILE* fp, int state){
	...
	for(int i = 0; i < lr_states[state]->nonkernel_items_count; i++){
		if(/*(lr_states[state]->nonkernel_items[i]->position == rules[lr_states[state]->nonkernel_items[i]->rule]->body_length) ||*/ (strcmp(rules[lr_states[state]->nonkernel_items[i]->rule]->body[0]->token_id, "EMPTY") == 0)){	
